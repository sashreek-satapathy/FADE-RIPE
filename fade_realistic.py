...
def lindblad_fidelity_and_grad(
    ...
    """J = Tr[rho_target^dagger rho(T)] - leak_lambda * Tr[P_leak rho(T)],
    with the EXACT adjoint-sweep gradient dJ/duI[k], dJ/duQ[k].
    Returns (J, gI, gQ, leakage_population_at_T).

    `steps`, if given, must be prop.step_data(uI, uQ, dt) already computed
    by the caller -- this is the expensive part (one 48x48 expm per
    timestep), and it depends only on (uI, uQ, prop), never on rho0/
    rho_target. Callers evaluating several states against the same
    control sequence (see average_gate_fidelity_and_grad) MUST compute it
    once and pass it in, or they pay for it redundantly per state."""
    Nt = len(uI)
    if steps is None:
...
def thermal_gate_fidelity_and_grad(
    ...
    """Evaluate/differentiate the gate objective starting from rho_th
    instead of the pure ground state, demonstrating (and optimizing
    against) resilience to thermal initialization error."""
...
def average_gate_fidelity_and_grad(
    ...
    """Standard average-gate-fidelity estimator: average the state
    fidelity/gradient over a small operator basis of computational-
    subspace input states (|0>,|1>,|+>,|+i>), each still initialized with
    the thermal population spread across the *other* (non-computational)
    levels via rho_th's higher-level occupations. This is the fairer
    figure of merit for reporting a single number in the paper; use
    thermal_gate_fidelity_and_grad() alone if you specifically want to
    show "here is what a |0>-started thermal run looks like."
    """
    N = sys_.N
    rho_th = sys_.thermal_state()
    leak_bg = np.real(np.trace(sys_.P_leak @ rho_th))  # pre-existing thermal leakage pop.
    ...
    Nt = len(uI)
    # step_data depends only on (uI, uQ, prop) -- compute ONCE and reuse
    # across all 4 kets. Previously each ket recomputed the full Nt x 48x48
    # exponential sweep from scratch, a ~4x redundant cost that dominated
    # this function's runtime (measured: step_data alone is ~95% of the
    # total wall time of a single lindblad_fidelity_and_grad call).
    steps = prop.step_data(uI, uQ, dt)

    J_tot, gI_tot, gQ_tot, leak_tot = 0.0, np.zeros(Nt), np.zeros(Nt), 0.0
    for psi in kets:
        rho_comp = np.outer(psi, psi.conj())
        # mix in the thermal background on levels >=2 (renormalized so the
        # computational-block population + thermal-leak population sums to 1)
        p_comp = 1.0 - leak_bg
        rho_in = p_comp * rho_comp + sys_.P_leak @ rho_th @ sys_.P_leak
        rho_target = sys_.ideal_target_dm(rho_in)
        J, gI, gQ, leak = lindblad_fidelity_and_grad(
            uI, uQ, rho_in, rho_target, prop, dt, cfg.leak_lambda, steps=steps)
        J_tot += J; gI_tot += gI; gQ_tot += gQ; leak_tot += leak
    n = len(kets)
    return J_tot / n, gI_tot / n, gQ_tot / n, leak_tot / n
...
def _ff_single_freq_term(om, dom, Sw, Ulist, Sk_list, steps, tlist, B, N, Nt, dt):
    """The O(Nt^2) contribution of ONE frequency to J_pen/gI/gQ. Factored
    out so the serial path and the parallel-worker path share the exact
    same math (no risk of the two implementations silently drifting
    apart)."""
...
def _selftest_gradients(verbose: bool = True) -> bool:
    ...
    def J_of(uI_, uQ_):
        J, _, _, _ = lindblad_fidelity_and_grad(
            uI_, uQ_, rho0, rho_target, prop, dt, cfg.leak_lambda)
        return J

    _, gI, gQ, _ = lindblad_fidelity_and_grad(
    ...
    if verbose:
        ...
        print(f"  Filter-function overlap term    : max err = {err_ff:.3e}")
        print(f"  PASS (< {tol:g})" if ok else f"  FAIL (>= {tol:g}) -- DO NOT TRUST L-BFGS-B RESULTS")
        print("="*70)
    return ok
...
def _fade_noise_chunk_worker(args):
    ...
    for d, g in zip(deltas_chunk, gammas_chunk):
        ...
        for psi in kets:
            ...
            J, gI, gQ, _ = lindblad_fidelity_and_grad(
...
def fade_ripe_objective_and_grad(uI_raw: np.ndarray, uQ_raw: np.ndarray,
    ...
    if parallel and K >= _N_WORKERS:
        ...
        for d, g in zip(deltas, gammas):
            ...
            J_k, gI_k, gQ_k, leak_k = average_gate_fidelity_and_grad(uI_f, uQ_f, prop_d, dt)
...
def fade_ripe_optimize(seed_uI: np.ndarray, seed_uQ: np.ndarray,
    ...
    J_fid_seed, _, _, leak_seed = average_gate_fidelity_and_grad(uI_seed_f, uQ_seed_f, prop, cfg.dt)
    ...
    rho_th = sys_.thermal_state()
    J_final, _, _, leak_final = thermal_gate_fidelity_and_grad(best_uI, best_uQ, prop, cfg.dt)
    if verbose:
...
def grape_optimize(verbose: bool = True) -> Tuple[np.ndarray, np.ndarray, float]:
    ...
    for restart in range(cfg.grape_restarts):
        ...
        prop = _get_propagator()
        F_r, _, _, _ = thermal_gate_fidelity_and_grad(uI_r, uQ_r, prop, cfg.dt)
        if verbose:
...
def analyze_noise_robustness(uI: np.ndarray, uQ: np.ndarray, run_id: str = "default"):
    """Analyze noise robustness using existing noise_eval machinery."""
    print("\n" + "="*70)
    print("7. NOISE ROBUSTNESS ANALYSIS")
    print("="*70)
    
    fidelities = noise_eval.fidelity_distribution(uI, uQ)
    deltas = noise_eval.deltas
    gammas = noise_eval.gammas
    ...
    stats = {
        ...
        'percentile_10': float(np.percentile(fidelities, 10)),
        'percentile_90': float(np.percentile(fidelities, 90)),
        'percentile_95': float(np.percentile(fidelities, 95)),
        'CVaR_5%': float(np.mean(fidelities[fidelities <= np.percentile(fidelities, 5)])),
        'CVaR_10%': float(np.mean(fidelities[fidelities <= np.percentile(fidelities, 10)])),
    ...
    noise_data = {
        'detuning_error': deltas,
        'amplitude_error': gammas,
        'fidelity': fidelities,
    }
    ...
    ax.set_ylabel('Count')
    ax.set_title('Fidelity distribution under quasi-static noise')
    ax.legend()
    save_figure(fig, f"fidelity_distribution_{run_id}")
    
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.scatter(deltas, fidelities, alpha=0.5, s=30)
    ax.set_xlabel('Detuning error (rad/ns)')
    ax.set_ylabel('Fidelity')
    ax.set_title('Fidelity vs detuning error')
    save_figure(fig, f"fidelity_vs_detuning_{run_id}")
    
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.scatter(gammas, fidelities, alpha=0.5, s=30)
    ax.set_xlabel('Amplitude error (relative)')
    ax.set_ylabel('Fidelity')
    ax.set_title('Fidelity vs amplitude error')
    save_figure(fig, f"fidelity_vs_amplitude_{run_id}")
    
    fig, ax = plt.subplots(figsize=(10, 8))
    sc = ax.scatter(deltas, gammas, c=fidelities, cmap='viridis', s=50)
    plt.colorbar(sc, label='Fidelity')
    ax.set_xlabel('Detuning error (rad/ns)')
    ax.set_ylabel('Amplitude error (relative)')
    ax.set_title('2D fidelity map under quasi-static noise')
    save_figure(fig, f"fidelity_2d_map_{run_id}")
    
    print(f"  Noise robustness statistics:")
...
def _common_method_metrics(uI: np.ndarray, uQ: np.ndarray,
    ...
    """Compute the SAME set of metrics for any method's final (uI, uQ) pulse
    pair, so FADE-RIPE, GRAPE, and CRAB land in analyze_method_comparison()
    on equal footing -- same thermal fidelity definition, same fixed
    out-of-sample noise ensemble (noise_eval), same leakage definition, same
    filter-function/PSD overlap penalty. Without this, GRAPE/CRAB were only
    ever scored on final_fidelity while FADE-RIPE was scored on everything
    it was optimized against -- not a fair 'same limits' comparison."""
    prop = _get_propagator()
    final_fidelity, _, _, leakage = thermal_gate_fidelity_and_grad(uI, uQ, prop, cfg.dt)
...
def run_benchmark(label: str):
    ...
    prop = _get_propagator()
    F_avg, _, _, leak_avg = average_gate_fidelity_and_grad(fade_uI, fade_uQ, prop, cfg.dt)
...
def main():
    ...
    parser.add_argument('--mode', choices=['single', 'selftest', 'compare-qtrl', 'analyze'],
                         default='single')
    parser.add_argument('--gate-time', type=float, default=180.0)
    parser.add_argument('--nt', type=int, default=None)
    parser.add_argument('--run-id', type=str, default="default")
    parser.add_argument('--workers', type=int, default=None,
                        help="Number of workers (auto-detects cores if not set)")
    ...
    if args.workers is not None:
        ...
        print(f"  Using {_N_WORKERS} workers (user-specified)")
    ...
    Nt = args.nt if args.nt is not None else max(8, int(round(args.gate_time / 0.5)))
    set_gate_time(Tg=args.gate_time, Nt=Nt)

    if args.mode == 'single':
        run_benchmark(f"{args.gate_time:.0f}ns transmon")
    elif args.mode == 'compare-qtrl':
        print("\nRunning qutip_qtrl GRAPE/CRAB in Lindbladian (GEN_MAT) mode "
              "for comparison against FADE-RIPE...")
        run_optimization_with_analysis(
            gate_time=args.gate_time,
            nt=args.nt,
            run_id=args.run_id,
            run_mode='compare-qtrl'
        )
